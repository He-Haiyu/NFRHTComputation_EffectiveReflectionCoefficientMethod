...
Tt = 331; % top structure temperature
Tb = 351; % bottom structure temperature
...
dThetaomgbt = ThetaomgTb - ThetaomgTt;
...
qprs = trapz(omg,qpromgs);
qprp = trapz(omg,qpromgp);
qevs = trapz(omg,qevomgs);
qevp = trapz(omg,qevomgp);

% print heat fluxes
disp('In: W/(m^2 rad/s)')
disp(['Q = ',num2str(qprs+qprp+qevs+qevp),', Qs = ',num2str(qprs+qevs),', Qp = ',num2str(qprp+qevp)]);
disp(['Qpr = ',num2str(qprs+qprp),', Qprs = ',num2str(qprs),', Qprp = ',num2str(qprp)]);
disp(['Qev = ',num2str(qevs+qevp),', Qevs = ',num2str(qevs),', Qevp = ',num2str(qevp)]);

%% Initialize figure
%Spectral_q
fignm = ['SpecQ: ' 'gp=' num2str(gp) ' Tt=' num2str(Tt) ', Tb=' num2str(Tb)];
...
plot(ah,omg,abs(qpromgs+qpromgp),'LineWidth',1.5);
plot(ah,omg,abs(qevomgs+qevomgp),'LineWidth',1.5);
