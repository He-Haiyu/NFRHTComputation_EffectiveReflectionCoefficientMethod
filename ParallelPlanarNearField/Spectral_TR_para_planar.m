...
for j = 1:length(omg)
    ...
    TRpromgs(j) = integral(@(x) Tprs(x).*x,0,k0j,'RelTol',rTol,'AbsTol',aTol);
    TRpromgp(j) = integral(@(x) Tprp(x).*x,0,k0j,'RelTol',rTol,'AbsTol',aTol);
    TRevomgs(j) = integral(@(x) Tevs(x).*x,k0j,k0j+8*pi/gp,'RelTol',rTol,'AbsTol',aTol);
    TRevomgp(j) = integral(@(x) Tevp(x).*x,k0j,k0j+8*pi/gp,'RelTol',rTol,'AbsTol',aTol);
...
% Conductance
TR = TRpromgp+TRpromgs+TRevomgp+TRevomgs;

TRprs = trapz(omg,TRpromgs);
TRprp = trapz(omg,TRpromgp);
TRevs = trapz(omg,TRevomgs);
TRevp = trapz(omg,TRevomgp);

% print heat fluxes
disp(['TR = ',num2str(TRprs+TRprp+TRevs+TRevp),', TRs = ',num2str(TRprs+TRevs),', TRp = ',num2str(TRprp+TRevp)]);
disp(['TRpr = ',num2str(TRprs+TRprp),', TRprs = ',num2str(TRprs),', TRprp = ',num2str(TRprp)]);
disp(['TRev = ',num2str(TRevs+TRevp),', TRevs = ',num2str(TRevs),', TRevp = ',num2str(TRevp)]);

%% Initialize figure
%Spectral_TR
fignm = ['SpecTR: ' 'gp=' num2str(gp)];
...
%% Plot
plot(ah,omg,abs(TR),'LineWidth',1.5);
plot(ah,omg,abs(TRpromgs+TRpromgp),'LineWidth',1.5);
plot(ah,omg,abs(TRevomgs+TRevomgp),'LineWidth',1.5);
